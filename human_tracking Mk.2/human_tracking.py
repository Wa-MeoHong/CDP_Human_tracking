...
def move_robot_BLDC():
    ...
    # forward (전진)
    if ((y>=(stop_range*2))):                       # y가 전진범위 (stop_range * 2 너머)에 있으면
...
"""
    2)서보모터를 움직이는 쓰레드
        Stop_flag => 서보모터 동작 (0), 서보모터 정지 (1)
        Servo_state => 0 : normal, 1 : left, 2 : right
            
        2-1) Stop_flag가 올라왔음 ( Stop_flag == 1)
             - 서보모터를 회전시켜 바퀴축을 1자로 만듦

        2-2) Stop_flag가 올라오지 않았음 ( Stop_flag == 0)
            2-2-1) 중심(빨간점)이 정지범위에서 좌측으로 벗어남 ( > stop_range )
             - 서보모터를 시계반대방향으로 회전시킴 ( 좌회전 )
             if) 중심이 정지범위 안에 들어옴
                - 서보모터를 회전시켜 바퀴를 1자로 만든다.

            2-2-2) 중심(빨간점)이 정지범위에서 우측으로 벗어남 ( < -stop_range )
             - 서보모터를 시계방향으로 회전시킴 ( 우회전 )
             if) 중심이 정지범위 안에 들어옴
                 - 서보모터를 회전시켜 바퀴를 1자로 만든다.

            2-2-3) 중심(빨간점)이 정지범위에 있음
             - 서보모터를 회전시켜 바퀴를 1자로 만든다.
"""
